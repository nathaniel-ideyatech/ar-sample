...
extension MapController: ARSCNViewDelegate {
...
extension MapController: MGLMapViewDelegate {
    ...
    func mapView(_ mapView: MGLMapView, didFinishLoading style: MGLStyle) {
        ...
        let colorStops = ["small": NSExpression(forConstantValue: color.withAlphaComponent(0.75)),
                          "big": NSExpression(forConstantValue: color)]
        circleStyleLayer.circleColor = NSExpression(format: "mgl_interpolate:withCurveType:parameters:stops:(waypoint-type, 'linear', nil, %@)", colorStops)
        let sizeStops = ["small": NSExpression(forConstantValue: 2),
                         "big": NSExpression(forConstantValue: 4)]
        circleStyleLayer.circleRadius = NSExpression(format: "mgl_interpolate:withCurveType:parameters:stops:(waypoint-type, 'linear', nil, %@)", sizeStops)
